...
var ProfileViewEffects = /** @class */ (function () {
    function ProfileViewEffects(actions$, router, userService) {
        ...
        this.ProfileRequest$ = this.actions$
            ...
            .pipe(operators_1.map(function (action) { return action.payload; }), operators_1.map(function (data) { return new profile_view_actions_1.GetProfileStart(); }));
        this.getProfile$ = this.actions$
            .ofType(profile_view_actions_1.ProfileViewActionTypes.GET_PROFILE_START)
            .pipe(operators_1.map(function (action) { return action.payload; }), operators_1.switchMap(function (data) { return _this.userService.getProfileInformation(); }), operators_1.map(function (res) { return new profile_view_actions_1.GetProfileSucceed(res); }), operators_1.catchError(function () { return Observable_1.Observable.of(new profile_view_actions_1.GetProfileFailed()); }));
        this.refreshUserInfo$ = this.actions$
            .ofType(profile_view_actions_1.ProfileViewActionTypes.GET_PROFILE_SUCCEED)
            .pipe(operators_1.map(function (action) { return action.payload; }), operators_1.map(function (data) { return new dashboard_1.RefreshUserInfoAction(data); }));
    }
